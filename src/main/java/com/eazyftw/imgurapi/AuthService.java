...
import com.eazyftw.imgurapi.model.OAuth2;
...
public class AuthService {
	...
	/**
	 * This makes ImgurApi trade an Imgur OAuth2 authorization code
	 ...
	 * @return a refresh token to be saved and passed to ImgurApi
	 *     in the future.
	 * @throws ImgurAuthException unable to refresh tokens
	 */
	public String setAuthorizationCode( String authCode ) throws ImgurAuthException {
		this.oAuth2 = null;
	 ...
	 * @throws ImgurAuthException whomp whomp
	 */
	public boolean isAccessTokenValid()  throws ImgurAuthException {
		if( oAuth2 == null ) {
			return false;
		} // if
		Call<Object> call =
	 ...
	 * @throws ImgurApiException the account couldn't be loaded
	 */
	public Account getAuthenticatedAccount() throws ImgurApiException {
		if( oAuth2 == null ) {
			return null;
		} // if
	...
	private boolean tradeAuthCodeForTokens( String authCode ) throws ImgurAuthException {
		...
		try {
			...
			oAuth2 = res.body();
			if( oAuth2 == null ) {
				throw new ImgurAuthException( res.message(), res.code() );
			} // if

			return true;
		} catch (IOException e) {
			throw new ImgurAuthException( "Error fetching tokens from authorization code: " + e.getMessage() );
		} 

	} // tradeAuthCodeForTokens
	
	private boolean updateAccessToken() throws ImgurAuthException {
		if( oAuth2 != null && !oAuth2.isExpiringSoon() ) {
			return true; // nothing to do! all's well as far as we know.
		} // if
		
		if( refreshToken == null ) {
			throw new ImgurAuthException( "Cannot update OAuth2 access token; need refreshToken or authorizationCode to be set. See setRefreshToken() or setAuthorizationCode().  User-authenticated calls will not work until then.");
		} // if
		...
		try {
			...
			throw new ImgurAuthException( "Error updating access tokens from refresh token: " + e.getMessage() );
		} 
	} // updateAccessToken

	/**
	 * Returns the ImgurApiClient (mostly internal use)
	 * @return the client
	 */
	public ImgurApiClient getClient() {
		return client;
	}
	 ...
	 * ImgurApi synchronously contacts Imgur to get an access token.
	 * Each client instance can have only one authenticated user at a time.
	 * @param refreshToken The user's refresh token.
	 * @throws ImgurAuthException couldn't fetch an access token
	 */
	public void setRefreshToken( String refreshToken ) throws ImgurAuthException {
	...
	protected AuthService(ImgurApiClient client, String clientId, String clientSecret ) {
		this.client = client;
